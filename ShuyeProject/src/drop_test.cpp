...
#define NPY_SUPPORT 0//不使用npy文件
#if NPY_SUPPORT
#include "cnpy/cnpy.h"
using namespace cnpy;
#endif

extern shuye_t shuye_state;

//本例为rknn官方示例下调整函数结构得来
...
int drop_detect(int argc, char **argv)
{
    ...
    for (int s = 0; s < shape_num; ++s)
    {
        ...
        for (int i = 0; i < io_num.n_input; i++)
        {
            ...
            inputs[i].buf = resize_imgs[i].data;
            inputs[i].size = resize_imgs[i].total() * resize_imgs[i].channels();
        }
        ...
        if(shuye_state.mode==1)
        {
            shuye_state.droptime=0;
            shuye_state.existing=0;
            shuye_state.dropping=0;
            shuye_state.dropped=0;
        }
        
        // 释放输出缓冲区数据，调用一次rknn_outputs_get后，必须调用一次rknn_outputs_release
